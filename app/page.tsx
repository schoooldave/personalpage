...
function PointerEffects() {
  ...
  useEffect(() => {
    ...
    const leave = () => { target.current = { x: -100, y: -100 }; };
    ...
    let frame = requestAnimationFrame(function animate() {
      positions.current.forEach((position, index) => {
        ...
        position.x += (source.x - position.x) * (index === 0 ? 0.24 : 0.16);
        position.y += (source.y - position.y) * (index === 0 ? 0.24 : 0.16);
    ...
    return () => { cancelAnimationFrame(frame); window.removeEventListener("pointermove", move); window.removeEventListener("pointerleave", leave); };
  }, []);
...
export default function Home() {
  const [world, setWorld] = useState<"personal" | "work">("work");
  ...
  return <main className={`${styles.site} ${world === "work" ? styles.workWorld : styles.personalWorld}`}>
    ...
    <nav className={styles.nav} aria-label="主导航">
      ...
      <div className={styles.navLinks}><a href="#cases">工作档案</a>{world === "work" && <a href="/notes">Field Notes</a>}<a href="/about">关于我</a><button type="button" onClick={toggleWorld}>{world === "personal" ? "进入工作世界 ↗" : "回到个人世界 ↗"}</button></div>
    </nav>
    <section className={styles.hero} id="about">
      <p className={styles.eyebrow}>{world === "personal" ? "PERSONAL CONSTELLATION / 01" : "MODERN FIELD NOTES / 01"}</p>
      <h1>{world === "personal" ? <>我把复杂的现场，<br />整理成可运行的系统。</> : <>从一线，理解问题；<br />在系统，解决问题；<br />让管理，与系统共变。</>}</h1>
      <p className={styles.heroCopy}>{world === "work" ? "从一线现场识别问题，在系统中形成方案，让管理机制同步改变。" : portfolioSummary.proof}</p>
      <div className={styles.heroActions}><a className={styles.primaryAction} href="#cases">查看代表项目 <span>↘</span></a><button className={styles.textAction} type="button" onClick={toggleWorld}>切换视角</button></div>
    ...
    {world === "work" && <section className={styles.careerArc} aria-label="能力路径"><div className={styles.arcTitle}><span>01</span><p>能力路径<br /><small>CAPABILITY ARC</small></p><h2>从业务洞察，<br />到系统化解题</h2><p className={styles.arcIntro}>在 FMCG 一线业务与数字化实践之间，形成从问题识别、系统构建到管理协同的能力路径。</p></div><div className={styles.arcSteps}><div><b>业务洞察</b><strong>从一线业务中识别关键问题</strong><p>通过一线市场与终端执行，理解渠道、门店、销售与经销商之间的真实业务关系，结合业务目标、执行流程与现场结果，识别影响落地的关键差距。</p><em>一线业务实践 · 2020—2022<br />代表实践：终端门店执行 · 渠道现场反馈</em></div><div><b>数字化构建</b><strong>将业务问题转化为系统能力</strong><p>从业务流程、角色协同与数据关系出发，完成需求分析、方案设计、产品协同与上线验证，将一线问题转化为可运行、可追踪的产品与系统方案。</p><em>数字化产品与系统实践 · 2022—2023<br />代表实践：SFA 深度迭代 · 数据权限治理</em></div><div><b>管理变革</b><strong>推动管理机制与数字化能力协同演进</strong><p>围绕业务目标、组织协同与数据反馈，推动管理规则与工作方式持续调整，使数字化能力从系统应用延伸到日常管理，并形成可持续的运行机制。</p><em>产品经理职能 · 2023—2025<br />代表实践：DMS 全国渠道分销透明化治理</em></div></div></section>}
    ...
    <section className={styles.cases} id="cases">
      ...
      <div className={styles.caseGrid}>{workCases.map((item, index) => <article className={styles.caseCard} key={item.id}><p className={styles.cardIndex}>0{index + 1} / 02</p><p className={styles.cardPeriod}>{item.period}</p><h2>{item.title}</h2><div className={styles.cardMetric}><strong>{item.metric}</strong><span>{item.metricLabel}</span></div><a href={`/cases/${item.id}`}>打开项目档案 <span>→</span></a></article>)}</div>
    </section>
    <section className={styles.recruiterBar} aria-label="招聘方快速阅读">
