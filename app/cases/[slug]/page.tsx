  ...
  return <main className={styles.page}>
    ...
    <section className={styles.content}>
      ...
      {isDms ? <>
        ...
        <div className={styles.block}><p className={styles.label}>07 / OUTCOME & METRIC</p><div><h2>结果如何变化</h2><ul>{dms.outcomes.map((outcome) => <li key={outcome}>{outcome}</li>)}</ul><p className={styles.note}>阶段目标和统计口径曾调整；约 80%为整体事业部口径，内部不同事业部存在差异，页面不展开逐项比较。</p></div></div>
