...
export default function CasePage({ params }: { params: { slug: string } }) {
  ...
  return <main className={styles.page} lang={locale === "zh-CN" ? "zh-CN" : locale}>
    <nav className={styles.nav}><Link href={localizePath("/", locale)}>{chrome.nav.home}</Link><div><LanguageSwitcher locale={locale} pathname={`/cases/${params.slug}`} /> <Link href={localizePath("/#cases", locale)}>{chrome.nav.backToCases}</Link></div></nav>
    ...
    </section><footer className={styles.footer}><Link href={localizePath(`/cases/${otherSlug}`, locale)}>{chrome.nextProject}{chrome.otherLabels[otherSlug]} <span>→</span></Link><Link href={localizePath("/", locale)}>{chrome.backHome}</Link></footer>
  </main>;
}
