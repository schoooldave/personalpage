...
export default function AboutPage() {
  ...
  return <main className={styles.page} lang={locale === "zh-CN" ? "zh-CN" : locale}>
    <nav className={styles.nav}><Link href="/">DAVE / FIELD NOTES</Link><div><LanguageSwitcher locale={locale} pathname="/about" /> <Link href="/">{copy.nav.personal}</Link></div></nav>
    ...
    <footer className={styles.footer}><Link href="/">{copy.footer.home}</Link><Link href="/cases/dms">{copy.footer.cases}</Link></footer>
  </main>;
}
