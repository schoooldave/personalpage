...
export default function PersonalWorld({ onToggle, copy }: { onToggle: () => void; copy: SiteContent["personalWorld"] }) {
  ...
  return <div className={styles.personalWorldContent}>
    ...
    <section className={styles.ending}><div><h2>{copy.ending.title.split("\n")[0]}<br />{copy.ending.title.split("\n")[1]}</h2><p>{copy.ending.body}</p><div className={styles.endingLinks}><a href="/about">{copy.ending.about} ↗</a><button type="button" onClick={onToggle}>{copy.ending.work} ↗</button></div></div><span>{copy.ending.signature}</span></section>
  </div>;
}
